...
def main() -> None:
    ...
    parser.add_argument(
        "--instruction-topic",
        default="/opentau/pi05_instruction",
        help="ROS std_msgs/String topic for pi0.5 instructions.",
    )
    parser.add_argument(
        "--stop-topic",
        default="/opentau/pi05_stop",
        help="ROS std_msgs/String topic for stop commands.",
    )
    parser.add_argument(
        "--cycle-period-s",
        type=float,
    ...
    parser.add_argument(
        "--publish-json",
        action="store_true",
        help="Publish JSON payloads instead of plain instruction strings.",
    )
    parser.add_argument(
        "--max-cycles",
        type=int,
